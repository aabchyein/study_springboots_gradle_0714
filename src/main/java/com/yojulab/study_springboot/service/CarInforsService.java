...
public class CarInforsService {
    ...
    public Map selectSearchWithPagination(Map dataMap) {
        // 페이지 형성 위한 계산
        int totalcount = (int) this.selectTotal(dataMap); // totalCount를 가져오기 위함.

        int currentPage = 1;  // 원래 parameter로 받는 값(클라이언트로부터 받는 값) = from client in param
        if(dataMap.get("currentPage") != null) {
            currentPage = Integer.parseInt((String)dataMap.get("currentPage"));    // from client in param
        }
        
        Paginations paginations = new Paginations(totalcount, currentPage);  // totalCount는 쿼리문을 갔다 와야 함, currentPage는 임의적으로 1로 넣어줌
        HashMap result = new HashMap<>();
        result.put("paginations", paginations); // page에 대한 정보
